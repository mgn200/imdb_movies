RSpec.describe ImdbPlayfield::TMDBApi do
  ...
  let(:tmdb) { ImdbPlayfield::TMDBApi.new }
  let(:movies) { ImdbPlayfield::MovieCollection.new.filter(title: 'Fight Club') }

  before do
    stub_const("ImdbPlayfield::TMDBApi::YML_FILE_PATH", File.expand_path("spec/yml_data/test_movies_info.yml"))
  end
  ...
  describe "#fetch_data" do
    it 'creates YML with TMDB movie info' do
      VCR.use_cassette('fight_club_tmdb_request') do
        ...
        yaml_data = YAML.load_file(ImdbPlayfield::TMDBApi::YML_FILE_PATH)
