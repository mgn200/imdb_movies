RSpec.describe ImdbPlayfield::IMDBScrapper do
  ...
  before { stub_const("ImdbPlayfield::IMDBScrapper::YML_FILE_PATH", File.expand_path("spec/yml_data/test_imdb_budget.yml")) }

  describe '#run' do
    it 'creates YML with budget info from imdb' do
      VCR.use_cassette('fight_club_html') do
        ImdbPlayfield::IMDBScrapper.run(movie)
        yaml_data = YAML.load_file(ImdbPlayfield::IMDBScrapper::YML_FILE_PATH)
